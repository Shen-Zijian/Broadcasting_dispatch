...
def driver_decision(distance, reward):
    ...
    for i in range(r_dis):
        for j in range(c_dis):
            ...
            if distance[i][j] > env_params['broadcasting_scale']:
                result[i, j] = 0  # delete drivers further than broadcasting_scale
...
def Plotting(x_list, y_list, name):
    z1 = np.polyfit(x_list, y_list, 3)  # 曲线拟合，返回值为多项式的各项系数
...
def dispatch_broadcasting(order_driver_info):
    ...
    driver_id_list = pd.Series(driver_id_list).unique()
    reward_list = order_driver_info['reward_units'].unique()
    order_driver_flag_list = order_driver_info['order_driver_flag'].unique()

    if min(len(driver_id_list), len(order_id_list)) != 0:
        ...
        for i in range(min(len(driver_id_list), len(order_id_list))):
